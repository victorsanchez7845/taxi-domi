@php
    ...
    $payment_sales = [
        "paid" => false,
        "sales" => $rez['sales']['total'],
        "payments" =>  $rez['payments']['total']
    ];
    
    if($payment_sales['payments'] >= $payment_sales['sales']):
        $payment_sales['paid'] = true;
    endif;
@endphp
            ...
            <div class="items">
                ...
                <div class="right">
                    ...
                    <div>
                        ...
                        <a href="@lang('link.quote_reservation_detail')?logout=true">{{ __('quote/reservation.close_session') }}</a>
                    </div>
                </div>
            </div>
        </div>
        @if($payment_sales['paid'] == false)
            <div class="container">
                <div class="alert">
                    @if(app()->getLocale() == "en")
                        <p>If you have already made your payment and it is still not reflected, click on the button to update</p>
                        <a href="@lang('link.quote_reservation_detail')?email={{ $rez['client']['email'] }}&code={{ reset($rez['items'])['code'] }}" title="Update">Update</a>
                    @else
                        <p>Si ya ha realizado el pago y aún no se ha reflejado, haga clic en el botón para actualizar</p>
                        <a href="@lang('link.quote_reservation_detail')?email={{ $rez['client']['email'] }}&code={{ reset($rez['items'])['code'] }}" title="Actualizar">Actualizar</a>
                    @endif
                </div>
            </div>
        @endif
    ...
    <div class="container information-section">
        ...
        <div class="right">
            ...
            @if($rez['config']['is_cancelled'] == false)
                @if(true)
                <a href="{{ __('link.payment') }}" class="stripe">
                    ...
                    <span>{{ __('quote/reservation.credit_card') }}</span>
                </a>
                @endif
                @if( $payment_link['STRIPE'] && false )
